...
std::string FirstAndLast(const std::string &str)
{
 char first = '\0', last = '\0';
 for(char c: str){
     if(isdigit(c)){
...
if (first != '\0'&& last != '\0'){
    return std::string(1,first) + last;
}else {
    return "";
}
}

int endvalue(const std::string &largestr){
    std::istringstream iss(largestr);
    std::string block;
    
    int finalValue = 0; 
    while(iss >> block){
        std::string digits = FirstAndLast(block);
        if (!digits.empty()){
            finalValue += std::stoi(digits);
        }
    } 
...
int main() {
    ...
    std::string largeString = R"(fivethreeonezblqnsfk1
two74119onebtqgnine
jrjh5vsrxbhsfour3
...
1rdtwofjvdllht5eightsixfourbl)";
    ...
    int result = endvalue(largeString);

    std::cout << "Sum of concatenated digits: " << result << std::endl;
